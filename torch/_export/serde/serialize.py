    ...
    if isinstance(s, (torch.SymInt, sympy.Symbol, int)):
        if symbolic_shapes.is_concrete_int(s):
            ...
            if s.node.hint is None:
                return SymInt.create(as_expr=SymExpr(expr_str="", expr_ast=sym_expr))
            else:
                return SymInt.create(
                    as_expr=SymExpr(expr_str="", expr_ast=sym_expr, hint=SymExprHint.create(as_int=s.node.hint))
                )
    else:
...
def serialize_sym_bool(s: Union[bool, torch.SymBool]) -> SymBool:
    if isinstance(s, (torch.SymBool, bool)):
        if symbolic_shapes.is_concrete_bool(s):
            ...
            return SymBool.create(as_expr=SymExpr(expr_str="", expr_ast=serialize_sym_expr(s)))
    else:
        raise SerializeError(
...
class GraphModuleDeserializer(metaclass=Final):
    ...
    def deserialize_sym_expr(self, s: SymExprNode, hint: Optional[int] = None, force_int: Optional[bool] = True) -> Union[sympy.Expr]:
        if (base := s.base) is not None:
            if base.type == "as_bool":
                ...
                if (
                    hint is not None
                    and not s in self.shape_env.var_to_val
                ):
